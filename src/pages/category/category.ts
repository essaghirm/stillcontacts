...
export class CategoryPage {
    ...
    action: any = null;
    loading: any
    constructor(private alertCtrl: AlertController, public navCtrl: NavController, public navParams: NavParams, public http: Http, private cp: ContactServicesProvider, public loadingCtrl: LoadingController) {
        this.getCategories()
    }
    ...
    cancel() {
        ...
        this.title = ""
        if(this.lvl < 6){
            this.getCategoriesByLvl(this['cv_' + (this.lvl-1)], this.lvl)
        }
    }
                    ...
                    handler: () => {
                        ...
                        this.http.delete(this.cp.url + 'category/' + this.category).map(res => res.json()).subscribe(
                            data => {
                                ...
                                this['categories_' + (this.lvl - 1)] = data.categories
                                this.categories_total = data.details.categories
                                this.contacts_total = data.details.contacts
                                this.cancel()
                                this.loadingDismiss()
