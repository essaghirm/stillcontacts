...
export class DetailPage {
	...
	loading: any
	constructor(private storage: Storage, private alertCtrl: AlertController, public viewCtrl: ViewController, public navCtrl: NavController, public navParams: NavParams, public http: Http, private socialSharing: SocialSharing, private callNumber: CallNumber, public actionSheetCtrl: ActionSheetController, private clipboard: Clipboard, private toastCtrl: ToastController, public popoverCtrl: PopoverController, private camera: Camera, public loadingCtrl: LoadingController, private cp: ContactServicesProvider) {
	...
	getDetails() {
		this.http.get(this.cp.url + 'contact/' + this.contact.id).map(res => res.json()).subscribe(
			data => {
				...
				this.setMostViewed(this.contact)
			},
			err => {
				console.log("Oops!")
			}
	...
	saveContact(type) {
		...
		if( (type == 'edit' && this.category == 0) || (type == 'add' && (this.category == null || this.category == 0)) ) {
			let toast = this.toastCtrl.create({
				message: 'vous n\'avez pas choisi la categorie, pour ajouter/modifier un contact, il faut choisir une categorie de niveau cinq.',
				cssClass: 'danger',
				duration: 5000,
				position: 'bottom'
			});
	...
	deletAvatar(){
		this.presentLoading()
		this.http.delete(this.cp.url+'contact/' + this.contact.id + '/avatar').map(res => res.json()).subscribe(
			data => {
				if(data == 'ok'){
					this.getDetails()
				}
